...
import 'result_screen.dart' show ScoreItem;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        _MatchRow(label: '핵심 증거', matched: matched.keyEvidences > 0,
...
class ResultPartsCard extends StatelessWidget {
  const ResultPartsCard({required this.matched, required this.missed, super.key});
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Text(text,
            style: AppText.body.copyWith(
                color: matched ? c.text : c.textSub, height: 1.5))),
      ]),
    );
  }
}
...
class ResultScoreCard extends StatelessWidget {
  const ResultScoreCard({required this.items, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: items.map((item) {
        final isDeduc = item.score < 0;
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: AppTokens.rowGap),
          child: Row(children: [
            Expanded(child: Text(item.label,
                style: AppText.body.copyWith(color: c.textSub))),
            Text(isDeduc ? '${item.score}점' : '+${item.score}점',
                style: AppText.monoNum.copyWith(
                    fontSize: AppTokens.fsXl, fontWeight: FontWeight.w600,
                    color: isDeduc ? c.danger : c.success, height: 1.0)),
          ]),
        );
      }).toList()),
    );
...
class ResultRevelationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(AppTokens.sp4),
      decoration: BoxDecoration(
        color: c.bg, border: Border.all(color: c.primarySoft),
        borderRadius: BorderRadius.circular(AppTokens.r4),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
